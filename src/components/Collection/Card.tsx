...
const Card : React.FC<{collections : Product[]}> = ({collections}) => {
  ...
  return (
    <nav className="w-screen flex items-center gap-2 overflow-x-auto h-36 overflow-y-hidden">
        {products?.map((product) => (
            <Link href={`/product/${product.id}`} className="bg-neutral-200 relative w-[140px] h-[150px] flex flex-col justify-center items-center  rounded-xl border-[1px] border-stone-600" key={product.id}>
